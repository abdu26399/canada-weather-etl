...
def test_flatten_shapes():
    sample = {
        ...
        "daily": [{"dt": 1700086400, "temp": {"day": 25, "min": 18, "max": 27, "night": 20, "eve": 23, "morn": 19},
                   "humidity": 55, "weather": [{"description": "ok"}]}]
    }
    df = flatten_city_payload("Toronto", sample)
    assert {"temp_actual", "temp_day", "temp_min", "temp_max"}.issubset(df.columns)
    assert not df.empty
